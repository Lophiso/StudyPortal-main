...
export async function runRealtimeIngestion() {
  ...
  // If no results were created/updated, insert a minimal static fallback so the UI is not empty
  if (results.length === 0) {
    console.log('[realtimeFetcher] no items ingested, inserting static fallback opportunities');

    const nowIso = new Date().toISOString();
    const staticItems = [
      {
        title: 'Sample PhD Position in Data Science',
        type: 'PHD',
        company: 'Example University',
        country: 'Unknown',
        city: 'Unknown',
        description:
          'Placeholder PhD opportunity used while live feeds are unavailable. Replace this with real data once the engine is fully configured.',
        requirements: ['Strong analytical skills', 'Background in statistics or CS'],
        deadline: null,
        postedAt: nowIso,
        applicationLink: 'https://example.org/sample-phd',
        source: 'static-fallback',
      },
      {
        title: 'Sample Industry Data Scientist Role',
        type: 'JOB',
        company: 'Example Tech Company',
        country: 'Unknown',
        city: 'Unknown',
        description:
          'Placeholder industry job used while live feeds are unavailable. Replace this with real data once the engine is fully configured.',
        requirements: ['Python', 'Machine Learning', 'SQL'],
        deadline: null,
        postedAt: nowIso,
        applicationLink: 'https://example.org/sample-job',
        source: 'static-fallback',
      },
    ];
    ...
    for (const s of staticItems) {
      const { error: upsertError } = await supabase
        .from('JobOpportunity')
        .upsert(s, { onConflict: 'applicationLink' });
